...
**目标：** 从代码和测试的模式中提取可复用的开发规范，持续提升团队代码稳定性和范式一致性。支持按前后端分别提取规范。
...
```text
<项目根目录>/.temp/standards-backfill-engine-<YYYYMMDD-HHMMSS>/
```
...
最终输出物**不保存在 skill 目录中**，而是保存到项目目录：`docs/quality-loop/<YYYYMMDD>/dev-standard.md`。
