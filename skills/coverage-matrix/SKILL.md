...
```text
<项目根目录>/.temp/coverage-matrix-<YYYYMMDD-HHMMSS>/
```
...
最终输出物**不保存在 skill 目录中**，而是保存到项目目录：`docs/quality-loop/<YYYYMMDD>/matrix.md`。
...
```json
{
  ...
  "rules": [
    ...
    },
    {
      ...
      "ownership": {
        "primary": "BE",
        "feResponsibility": "显示默认值提示",
        "beResponsibility": "降级逻辑处理"
      },
