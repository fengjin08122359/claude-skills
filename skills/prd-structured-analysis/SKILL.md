...
```text
<项目根目录>/.temp/prd-structured-analysis-<YYYYMMDD-HHMMSS>/
```
...
最终输出物**不保存在 skill 目录中**，而是保存到项目目录：`docs/quality-loop/<YYYYMMDD>/analysis.md`。
...
**触发条件：** 当使用 `--fe-be-split` 参数时执行此阶段

**划分规则：**

| 归属 | 判定标准 | 典型场景 |
|------|---------|---------|
| **FE** | 用户可见的交互、展示逻辑 | UI 状态联动、表单校验、路由跳转、组件渲染 |
| **BE** | 数据存储、业务规则、调度 | 持久化、唯一性校验、定时任务、异常处理 |
| **FE+BE** | 需要接口约定的协作 | 状态同步、文件上传、导出功能、数据校验 |
| **UNKNOWN** | PRD 描述不清、需确认 | 技术方案未定、业务逻辑模糊 |
...
```json
{
  ...
  "feBeSplit": {
    ...
    "frontendRules": ["NAV001", "NAV002", "UI001"],
    "backendRules": ["R001", "R002", "VAL001"],
    "collaborationRules": ["STATE001", "FMT001"],
    "unknownItems": [
      {
        "id": "U001",
        "question": "报告生成状态同步方式？",
        "options": ["WebSocket", "SSE", "轮询"],
        "needConfirmBy": "架构师"
      }
    ]
  }
